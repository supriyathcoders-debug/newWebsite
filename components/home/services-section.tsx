...
export function ServicesSection() {
  return (
    <section id="services" className="py-28 section-shell">
      <StaggerReveal className="mb-14 max-w-[720px]">
        <StaggerItem>
      ...
      <StaggerReveal className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {SERVICES.items.map((service) => (
          <StaggerItem key={service.title}>
            <article className="border border-border-subtle bg-brand-soft/30 hover:border-brand/25 transition-colors h-full flex flex-col overflow-hidden">
              <BrandImage
                src={service.image}
                alt={service.imageAlt}
                frame="subtle"
                className="border-0 border-b border-border-subtle"
              />
